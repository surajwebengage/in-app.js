    ...
    function selectRating(rating) {
        // Reset all rating options
        document.querySelectorAll('#rating-options input[type="radio"]').forEach(radio => {
            radio.checked = false;
        });

        // Set the selected rating
        document.querySelector(`#rating-options input[value="${rating}"]`).checked = true;
    ...
    document.querySelectorAll('#unhappy-reasons input[type="checkbox"]').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            if (this.value === 'Other Reason') {
                document.getElementById('unhappy-other-feedback').style.display =
    ...
    document.querySelectorAll('#awesome-reasons input[type="checkbox"]').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            if (this.value === 'Other Reason') {
                document.getElementById('awesome-other-feedback').style.display =
                    this.checked ? 'block' : 'none';
            }
            updateReasons();
        });
    ...
    function submitSurvey() {
        ...
        if (selectedRating === 'neutral') {
            ...
            if (reasons.includes('Other Reason')) {
                const otherFeedback = selectedRating === 'unhappy' ?
                    document.getElementById('unhappy-other-text').value.trim() :
                    document.getElementById('awesome-other-text').value.trim();
        ...
        const surveyData = {
            ...
            reasons: reasons,
            feedback: selectedRating === 'neutral' ?
                document.getElementById('neutral-textarea').value.trim() :
                otherFeedbackText
        };
